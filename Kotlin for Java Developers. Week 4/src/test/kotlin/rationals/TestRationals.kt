...
class TestRationals {
    ...
    private fun testNormalizedForm(rational: String, normalizedForm: String) {
        Assert.assertEquals(
            "Wrong normalized form for $rational",
            normalizedForm, rational.toRational().toString()
        )
    }

    private fun testArithmeticOperation(
        first: String, operationRepresentation: String, second: String,
        expectedResult: String, operation: Rational.(Rational) -> Rational
    ) {
        Assert.assertEquals(
            """Wrong result for $first $operationRepresentation $second""",
    ...
    private fun testComparison(
        ...
        Assert.assertEquals(
            "Wrong result for $first ${comparison.symbols} $second",
    ...
    private fun testInRange(element: String, start: String, end: String, expected: Boolean = true) {
        Assert.assertEquals(
            "Wrong result for $element in $start..$end",
    ...
    fun test0Sample6Integer() {
        Assert.assertEquals(
            "Wrong string representation for integer number",
    ...
    fun test0Sample7NormalizedForm() {
        Assert.assertEquals(
            "Wrong normalized form for '-2 divBy 4'",
            "-1/2", (-2 divBy 4).toString()
        )
        Assert.assertEquals(
            "Wrong normalized form for '117/1098'",
    ...
    fun test0Sample9InRange() {
        Assert.assertTrue(
            "Wrong result for checking belonging to a range",
    ...
    fun test0Sample10Long() {
        Assert.assertEquals(
            "Wrong result for normalization of '2000000000L divBy 4000000000L'",
    ...
    fun test0Sample11BigInteger() {
        Assert.assertEquals(
            "Wrong result for normalization of\n" +
    ...
    @Test
    fun test2Arithmetic4() = testArithmeticOperation(
        "828099487587993325537/44002379163849686934",
        "+",
        "597728771407450572129/542645811175759848891",
        "17617266896778903272923516079952426936739/884359508704835805965897828865092484822",
        Rational::plus
    )

    @Test
    fun test2Arithmetic5() = testArithmeticOperation(
        "17311206/15881920",
        "-",
        "349928488/277922736",
        "-8267885027/48896076960",
        Rational::minus
    )

    @Test
    fun test2Arithmetic6() = testArithmeticOperation(
        "2339496978/2849004564",
        "*",
        "6292023/4423077",
        "46121885693/39483160566",
        Rational::times
    )

    @Test
    fun test2Arithmetic7() = testArithmeticOperation(
        "-159839855495465822788/224723091004279289771",
        "/",
        "2199321524829342798673/4174073637464826992623",
        "-7781380285275554790183890063972035564/5764317318107961130856061713789364263",
        Rational::div
    )
    ...
    @Test
    fun test3Comparison2() = testComparison(
        "-1042438361047144366998/59812037109262381713",
        EQUAL,
        "1076615241954175969826/-61773005685895342531",
        true
    )

    @Test
    fun test3Comparison3() = testComparison(
        "-1042438361047144366998/59812037109262381713",
        MORE_OR_EQUAL,
        "-1076615241954175969826/61773005685895342531",
        true
    ...
    @Test
    fun test3Comparison4() = testComparison(
        "259877352047152420620/1282471666315533247320",
        LESS_OR_EQUAL,
        "1461828422172186708965/7213993515334933226490",
        true
    )

    @Test
    fun test3Comparison5() = testComparison(
        "259877352047152420620/1282471666315533247320",
        LESS,
        "1461828422172186708965/7213993515334933226490",
        false
    ...
    @Test
    fun test3Comparison8() = testComparison(
        "95676047876718598058/21886075122687372173",
        LESS,
        "719390507052102557245/53591306636978481949",
        true
    )

    @Test
    fun test3Comparison9() = testComparison(
        "-48673102662562360478/2261578507553402607",
        LESS_OR_EQUAL,
        "-3238921919908420813980/150495361935138744892",
        true
    )
    ...
    @Test
    fun test4InRange3() = testInRange(
        "20395802948019459839003802001190283020/" +
